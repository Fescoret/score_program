...
namespace AlphaTap
{
    public partial class MailChangeForm : Form
    {
        ...
        private void mailNew_Enter(object sender, EventArgs e)
        {
            if (passNew2.Text == "")
            {
                passNew2.ForeColor = Color.Gray;
                passNew2.Text = "Новая почта";
            }
        }
        ...
        private void buttonChange_Click(object sender, EventArgs e)
        {
            ...
            MySqlCommand command = new MySqlCommand("UPDATE `users` SET `mail` = @mail WHERE `login` = @login;", db.GetConnection());
            command.Parameters.Add("@mail", MySqlDbType.VarChar).Value = mailNew.Text;
            ...
            if (passNew1.Text == SessionData.user_pass || passNew2.Text == SessionData.user_pass)
            {
                if (passNew1.Text == passNew2.Text)
                {
                    ...
                    db.CloseConnection();
                    SessionData.user_mail = mailNew.Text;
                    this.Close();
                    MenuForm menuForm = new MenuForm();
                    menuForm.Show();
                }
